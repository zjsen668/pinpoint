...
public class DataSourceConstructorInterceptor implements AroundInterceptor {
    ...
    public void after(Object target, Object[] args, Object result, Throwable throwable) {
        ...
        if ((target instanceof DataSourceMonitorAccessor) && (target instanceof DruidDataSource)) {
            DruidDataSourceMonitor dataSourceMonitor = new DruidDataSourceMonitor((DruidDataSource)target);
            dataSourceMonitorRegistry.register(dataSourceMonitor);
            if(logger.isDebugEnabled()) {
                logger.debug("[DRUID] register dataSource monitor");
            }
            ((DataSourceMonitorAccessor) target)._$PINPOINT$_setDataSourceMonitor(dataSourceMonitor);
        }else{
            logger.debug("[DRUID] not register dataSource monitor");
        }
    }
